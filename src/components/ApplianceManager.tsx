...
interface ApplianceManagerProps {
  ...
  onClearAllAppliances?: () => void;
}

const ApplianceManager: React.FC<ApplianceManagerProps> = ({ 
  appliances = [], 
  onAddAppliance, 
  onRemoveAppliance, 
  ...
  const applianceTypes: Record<string, string[]> = {
    'Air Conditioner': ['Split Type Inverter 1.0HP', 'Split Type Inverter 1.5HP', 'Split Type Inverter 2.0HP', 'Window Type 1.0HP', 'Window Type 1.5HP', 'Window Type 2.0HP', 'Tower Type', 'Portable AC'],
    'Electric Fan': ['Standing Fan', 'Table Fan', 'Wall Fan', 'Ceiling Fan', 'Exhaust Fan', 'Tower Fan'],
    'Refrigerator': ['Single Door 5-7 cu.ft.', 'Two Door 8-12 cu.ft.', 'Side-by-Side 15-20 cu.ft.', 'French Door 20+ cu.ft.', 'Mini Refrigerator', 'Inverter Type', 'Non-Inverter Type'],
    'Television': ['LED TV 32"', 'LED TV 42"', 'LED TV 50"', 'Smart TV 55"', 'OLED TV 65"', 'CRT TV (Old)', 'Plasma TV'],
    'Rice Cooker': ['Standard 1.0L', 'Standard 1.8L', 'Fuzzy Logic 1.8L', 'Induction Heating 1.8L', 'Multi-cooker'],
    'Electric Kettle': ['1.0L Electric Kettle', '1.5L Electric Kettle', '1.7L Electric Kettle', 'Stainless Steel', 'Glass Type', 'Temperature Control'],
    'Microwave Oven': ['Solo 20L', 'Solo 23L', 'Grill 25L', 'Convection 28L', 'Inverter Type', 'Built-in Type'],
    'Washing Machine': ['Top Load Automatic 6kg', 'Top Load Automatic 8kg', 'Front Load 7kg', 'Front Load 10kg', 'Twin Tub 8kg', 'Semi-Automatic'],
    'Clothes Iron': ['Dry Iron 1000W', 'Steam Iron 1200W', 'Steam Iron 1500W', 'Travel Iron', 'Garment Steamer'],
    'Water Heater / Shower': ['Storage 15L', 'Storage 30L', 'Instant 3.5kW', 'Instant 4.5kW', 'Solar Water Heater', 'Point-of-Use'],
    'Laptop / Desktop Computer': ['Laptop Basic', 'Laptop Gaming', 'Desktop CPU Only', 'Desktop with Monitor', 'Gaming PC', 'Workstation', 'All-in-One PC'],
    'Lighting (Bulbs / LEDs)': ['LED Bulb 9W', 'LED Bulb 12W', 'LED Bulb 18W', 'LED Tube 20W', 'LED Panel 30W', 'Fluorescent 20W', 'Incandescent 60W'],
    'Mobile Phone Charger': ['Standard 5W', 'Fast Charger 18W', 'Fast Charger 33W', 'Wireless Charger 10W', 'Car Charger', 'Power Bank'],
    'Electric Stove / Induction Cooker': ['Single Burner 1200W', 'Double Burner 1800W', 'Induction Cooker 1800W', 'Induction Cooker 2200W', 'Glass Top Stove', 'Portable Hot Plate'],
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const getWattsForAppliance = (category: string, type: string) => {
  ...
  const transformedAppliances = appliances.map((app: any) => ({
    id: app.id || app._id,
      ...
      <div className="manager-grid">
        <div className="manager-form-section card">
          <div className="form-header">
            ...
            <p className="form-subtitle">Select category and specific type for accurate calculations</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="add-appliance-form">
            ...
            {selectedCategory && selectedCategory !== 'Other' && applianceTypes[selectedCategory] && (
              <div className="form-group">
                ...
                <p className="form-hint">Select the specific model for accurate power consumption</p>
              </div>
            )}
        ...
        <div className="manager-list-section">
          ...
          {transformedAppliances.length === 0 ? (
            <div className="empty-state">
              ...
              <h4>No Appliances Added</h4>
              <p>Start by adding your first appliance using the form on the left</p>
            </div>
          ) : (
            <div className="appliance-list">
              {transformedAppliances.map((appliance: any) => (
                <div key={appliance.id} className="appliance-item">
                  <div className="appliance-icon">{appliance.icon}</div>
                  <div className="appliance-info">
                    <h4>{appliance.name}</h4>
                    <div className="appliance-subtext">
                      <span className="appliance-category">{appliance.category}</span>
                      <span className="appliance-type">{appliance.type}</span>
                    </div>
                    <p className="appliance-details">
                      ⚡ {appliance.watts}W • ⏰ {appliance.hoursPerDay}h/day • 📅 {appliance.daysPerWeek} days/week
                    </p>
                  </div>
                  <button 
